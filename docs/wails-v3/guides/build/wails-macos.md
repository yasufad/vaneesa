...
Package your Wails application for macOS distribution.
...
This creates a single `.app` that runs natively on both architectures.
...
This uses `build/appicon.png` to produce `darwin/icons.icns` and `windows/icon.ico`.
...
**Tip:** Customize the DMG background and layout by editing `build/darwin/dmg/` assets.
