...
Limiting your app to a single running instance.
...
**Warning:** The Single Instance feature implements an optional encryption protocol using AES-256-GCM. Without encryption enabled, data passed between instances is not secure. When using the single instance feature without encryption, your app should treat any data passed to it from second instance callback as untrusted.

### Secure Communication

To enable secure communication between instances, provide a 32-byte encryption key:
...
When handling second instance launches, you'll often want to bring your application window to the front:
...
**macOS:** Single instance lock using a named mutex. The mutex name is generated from the unique id that you provide. Data is passed to the first instance via NSDistributedNotificationCenter.

**Windows:** Single instance lock using a named mutex. The mutex name is generated from the unique id that you provide. Data is passed to the first instance via a shared window using SendMessage.

**Linux:** Single instance lock using dbus. The dbus name is generated from the unique id that you provide. Data is passed to the first instance via dbus.
