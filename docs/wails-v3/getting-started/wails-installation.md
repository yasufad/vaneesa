...
**Note:** If you prefer a different package manager to npm, feel free to use it. You will need to update the project Taskfiles to use it.
...
Linux requires the standard `gcc` build tools plus `gtk3` and `webkit2gtk`. Run `wails doctor` after installation to be shown how to install the dependencies. If your distro/package manager is not supported, please let us know on discord.
