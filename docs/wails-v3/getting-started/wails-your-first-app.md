...
**Note:** Although Wails v3 uses [Task](https://taskfile.dev/) as its default build system, there is nothing stopping you from using `make` or any other alternative build system.
...
**Note:** `wails3 build` is shorthand for `wails3 task build` and will run the `build` task in `Taskfile.yml`.
...
For more detailed information about packaging options and configuration, check out our [Packaging Guide](/guides/packaging).
...
**Tip:** You can automate all of the initialisation steps by using the `-git` flag when creating your project:
```bash
...
If you are new to Wails, we recommend reading through our Tutorials next which will be a practical guide through the various features of Wails. The first tutorial is [Creating a Service](/tutorials/01-creating-a-service).
